...
export function StatusItem({ icon, label, detail, color }: StatusItemProps) {
  return (
    <div className="flex items-start gap-4 p-2 bg-white/40 rounded-2xl border border-white/60 group">
      <div className={cn("w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-[19.5px] shrink-0 transition-transform group-hover:scale-110", color)}>
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <h5 className="text-[12.5px] font-black text-stone-900 uppercase tracking-wide">{label}</h5>
